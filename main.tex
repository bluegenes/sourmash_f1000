...
\lstinline{sourmash} provides a user-friendly, extensible platform to enable MinHash signature generation and manipulation for DNA, RNA, and protein data. Sourmash is designed to facilitate containment queries for taxonomic exploration and identification while maintaining all functionality available via standard genomic MinHash techniques. % improve last phrase of this sentence / emphasize library functionality, etc!
...
\lstinline{sourmash} modifies standard genomic MinHash techniques by scaling the number of retained hashes to better represent and compare datasets of varying size and complexity, and by optionally tracking the abundance of each retained hash, to better represent data of metagenomic and transcriptomic origin and allow abundance comparisons.
...
All \lstinline{sourmash} comparisons work on signatures, compressed representations of biological sequencing data. To create a signature from contiguous sequences (i.e. a genome) with abundance tracking:
...
By default, the name of the file becomes the name of the signature. To name the signature from the first line of the sequencing file, use \lstinline{--name-from-first}. Although the \lstinline{--track-abundance} flag is optional, downstream comparison methods contain the flag \lstinline{--ignore-abundance}, or abundances can be removed with \lstinline{sourmash signature flatten}, thus we recommend calculating all signatures with abundance tracking. 
...
When using methods that compare a signature against a database such as \lstinline{gather} or \lstinline{search}, k-mer trimming is not recommended. These methods use exact matching of hashes in the signature to those in the databases. k-mer trimming may reduce the runtime of the search, but could increase false negatives. 
...
We can also use \lstinline{sourmash} with abundance tracking for tetranucleotide frequency clustering. Tetranucleotide usage is species-specific, with strongest conservation in DNA coding regions \cite{Pride2003}. This is often used in metagenomics as one method to "bin" assembled contiguous sequences together that are from the same species \cite{Albertsen2013}. Recently, tetranucleotide frequency clustering using \lstinline{sourmash} was used to detect microbial contamination in the domesticated olive genome \cite{Reiter2018}. Here we reimplement this approach using 100 of the 11,038 scaffolds in the draft genome. We will calculate the signature using a k-mer size of 4, use all 4-mers, and track abundance. Then we will use \lstinline{sourmash compare} to calculate the similarity between each scaffold. Using the \lstinline{--singleton} flag, we calculate a signature for each sequence in the fasta file.
...
MinHash comparisons are useful for outlier detection. Below we compare 50 genomes that contain the word "\texit{Escherichia coli}." We have pre-calculated the signatures for each of these genomes. We then use the \lstinline{plot} function to visualize our comparison.
...
\caption{\label{fig:compmatrix} Heatmap and dendrogram generated using \lstinline{sourmash} signatures built from 50 genomes that contained the word "\texit{Esherichia coli}." One signature is an outlier (highlighted in blue on the dendrogram).}
\end{figure}
...
First, we download a small set of k-mer trimmed \textit{Escherichia coli} reads and generate a signature with k=31.
...
The \lstinline{sourmash} package provides an ecosystem of tools to conduct sequence comparisons and taxonomic classification, and makes comparison against large-scale databases such as GenBank and SRA tractable. \lstinline{sourmash} signatures are small and irreversible, which means they can be used to facilitate pre-publication data sharing that may help improve classification databases and facilitate comparisons among similar datasets. 
